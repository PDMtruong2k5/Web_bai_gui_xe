...
<head>
    ...
    <style>
        body { background: linear-gradient(135deg, #4e54c8 0%, #8f94fb 100%); min-height:100vh; display:flex; align-items:center; padding:20px; }
        .form-container { background:white; border-radius:18px; padding:34px; box-shadow:0 10px 30px rgba(0,0,0,0.08); width:100%; max-width:640px; margin:auto; }
...
<body>
    <div class="container">
        <div class="form-container">
            <h3 class="form-title">CHỈNH SỬA KHU VỰC</h3>
            <?php
            ...
            <form action="../../handle/area_process.php" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" class="form-control" id="class_name" name="class_name" value="<?= htmlspecialchars($area['area_name']) ?>" placeholder="VD: Khu A - Tầng 1" required>
                    <div class="form-text">Tên khu vực hoặc vị trí</div>
                </div>

                <div class="mb-4">
                    <label for="school_year" class="form-label">Mô tả / Sức chứa</label>
                    <input type="text" class="form-control" id="school_year" name="school_year" value="<?= htmlspecialchars($area['area_desc']) ?>" placeholder="VD: 20 xe" required>
                    <div class="form-text">Ghi chú ngắn hoặc sức chứa tối đa</div>
                </div>

                <div class="d-flex gap-3 mt-4">
                    <button type="submit" class="btn btn-primary flex-grow-1"><i class="fas fa-save me-2"></i>Cập nhật</button>
