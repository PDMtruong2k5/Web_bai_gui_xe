...
function handleConfirmPayment() {
    if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_POST['transaction_id'])) {
        header("Location: ../views/transaction/admin_payment.php?error=Thiếu thông tin giao dịch");
        exit();
    }
    $transaction_id = (int)$_POST['transaction_id'];
    $success = confirmTransactionPayment($transaction_id);
    if ($success) {
        header("Location: ../views/transaction/admin_payment.php?success=Xác nhận thanh toán thành công");
    } else {
        header("Location: ../views/transaction/admin_payment.php?error=Có lỗi khi xác nhận thanh toán");
    }
    exit();
}
